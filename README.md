...
2. **动态链接优化**: Botan 3 加密库使用动态链接，减少二进制文件大小并提高安全性
3. **依赖管理**: 自动安装 KCPTube 所需的运行时依赖，包含加密库的动态库
4. **安全性**: 使用非特权用户运行容器
5. **多架构支持**: 支持 AMD64 和 ARM64 架构
6. **构建优化**: 包含 .dockerignore 和构建缓存优化
7. **错误处理**: 改进的构建错误处理和诊断
...
新版本包含以下改进：
- **加密库优化**: 从静态链接改为动态链接 Botan 3 库，提高安全性和减少镜像大小
- 安全扫描和漏洞检测
- 构建证明和供应链安全
- 改进的错误处理和重试机制
- Pull Request 构建测试
